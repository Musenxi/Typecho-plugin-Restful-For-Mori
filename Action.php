...
class Restful_Action extends Typecho_Widget implements Widget_Interface_Do
{
    ...
    public function postsAction()
    {
        ...
        foreach ($result as $key => $value) {
            ...
            if ($showDigest === 'more') {
                ...
                $result[$key] = $this->filter($result[$key], $parseMarkdown);
            ...
            if (!$showContent) {
                ...
                unset($result[$key]['markdown']);
                unset($result[$key]['html']);
            }
        }
    ...
    public function postAction()
    {
        ...
        if (count($result) != 0) {
            $result = $this->filter($result, $parseMarkdown);
    ...
    public function commentAction()
    {
        ...
        $select = $this->db->select('cid', 'created', 'type', 'slug', 'commentsNum', 'text', 'authorId')
            ->from('table.contents')
            ->where('password IS NULL');
        ...
        $postData = array(
            ...
            'ownerId' => isset($result['authorId']) ? $result['authorId'] : 0,
            'status' => $this->options->commentsRequireModeration ? 'waiting' : 'approved',
        );

        // Keep behavior consistent with Typecho native feedback:
        // if moderation is off but whitelist is on, only users with approved history are auto-approved.
        if (!$this->options->commentsRequireModeration && $this->options->commentsWhitelist) {
            $checkApproved = $this->db->select('coid')
                ->from('table.comments')
                ->where(
            ...
            if ($this->db->fetchRow($checkApproved)) {
    ...
    public function archivesAction()
    {
        ...
        foreach ($posts as $key => $post) {
            ...
            if ($showDigest === 'more') {
                ...
                $post = $this->filter($post, $parseMarkdown);
            } elseif ($showDigest === 'excerpt') {
                $limit = (int)trim($this->getParams('limit', '200'));
                $post = $this->filter($post, $parseMarkdown);
    ...
    /**
     * 是否对文章内容执行 markdown -> html 解析
     *
     * @return bool
     */
    private function shouldParseMarkdown()
    {
        $raw = strtolower(trim($this->getParams('parseMarkdown', 'true')));
        return !in_array($raw, array('false', '0', 'off', 'no'), true);
    }

    /**
     * 移除 Typecho markdown 标记
     *
     * @param mixed $content
     * @return string
     */
    private function stripMarkdownTag($content)
    {
        if (!is_string($content)) {
            return '';
        }

        return str_replace('<!--markdown-->', '', $content);
    }
    ...
    private function filter($value, $parseMarkdown = true)
    {
        ...
        $rawText = $this->stripMarkdownTag($value['text']);
        $rawDigest = $this->stripMarkdownTag($value['digest']);

        if ($parseMarkdown) {
            $digestSource = empty(trim($rawDigest)) ? $rawText : $rawDigest;
            $value['text'] = $this->safelyParseMarkdown($rawText);
            $value['digest'] = $this->safelyParseMarkdown($digestSource);
        } else {
            $digestSource = empty(trim($rawDigest)) ? $rawText : $rawDigest;
            $value['markdown'] = $rawText;
            $value['html'] = $this->safelyParseMarkdown($rawText);
            $value['text'] = $rawText;
            $value['digest'] = $digestSource;
        }
